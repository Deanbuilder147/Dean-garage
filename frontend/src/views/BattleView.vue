...
function renderBattlefield() {
  ...
  cells.forEach(cell => {
    ...
    graphics.setStrokeStyle({ width: 2, color: borderColor });
    ...
    // 绘制出生点标记
    if (isSpawnPoint) {
      const typeLabel = new Text(cell.terrain === 'mothership' ? '母舰' : '基地', {
        fontSize: 10,
        fill: '#fff'
      });
    ...
    // 出生点选择阶段：可点击的格子
    if (phase === 'spawn_selection' && isSpawnPoint && isMyTurn) {
      const cellArea = new Graphics();
      cellArea.fill({ color: 0xffffff, alpha: 0.001 }); // 几乎透明
      cellArea.circle(0, 0, hexSize * 0.9);
      cellArea
      cellArea.x = x;
      cellArea.y = y;
    ...
    if (phase === 'spawn_deployment') {
      ...
      if (mySpawn?.spawnPoint?.q === cell.q && mySpawn?.spawnPoint?.r === cell.r) {
        // 绘制可部署区域标识
        const deployMarker = new Graphics();
        deployMarker.setStrokeStyle({ width: 3, color: 0x00f260 });
        deployMarker.circle(0, 0, hexSize * 0.7);
        deployMarker.stroke();
        deployMarker.stroke();
        deployMarker.x = x;
        deployMarker.y = y;
        ...
        dropZone.fill({ color: 0x00f260, alpha: 0.3 });
        dropZone.circle(0, 0, hexSize * 0.9);
        dropZone.fill();
        dropZone.fill();
        dropZone
        dropZone.x = x;
  ...
  units.forEach(unit => {
    ...
    unitGraphic.fill({ color: color });
    unitGraphic.circle(0, 0, 20);
    unitGraphic
    ...
    unitGraphic.fill({ color: 0x20262f });
    unitGraphic.rect(-15, 25, 30, 5);
    unitGraphic.fill();
    unitGraphic.fill();
    unitGraphic

    unitGraphic.fill({ color: hpRatio > 0.5 ? 0x00f260 : hpRatio > 0.25 ? 0xff9800 : 0xf44336 });
    unitGraphic.rect(-15, 25, 30 * hpRatio, 5);
    unitGraphic

    // 隐匿图标
    if (hidden) {
      const hiddenIcon = new Text('👁️', {
        fontSize: 12,
        fill: '#fff'
      });
    ...
    // 名称
    const label = new Text(unit.name?.substring(0, 3) || 'Unit', {
      fontSize: 10,
      fill: '#fff'
    });
    ...
    if (unit.royroy_deployed) {
      const royroyGraphic = new Graphics();
      royroyGraphic.fill({ color: 0xffff00 });
      royroyGraphic.circle(0, 0, 12);
      royroyGraphic.fill();
      royroyGraphic.fill();
      royroyGraphic
...
let socketSurpriseInterval = null;
...
function setupSocket() {
  ...
  socket.on('surprise-timer-start', (data) => {
    ...
    if (socketSurpriseInterval) clearInterval(socketSurpriseInterval);
    socketSurpriseInterval = setInterval(() => {
      surpriseTimer.value--;
      if (surpriseTimer.value <= 0) {
...
onUnmounted(() => {
  if (surpriseTimerInterval) { clearInterval(surpriseTimerInterval); surpriseTimerInterval = null; }
  if (socketSurpriseInterval) { clearInterval(socketSurpriseInterval); socketSurpriseInterval = null; }
  if (socket) {
