...
router.get('/:id', authenticate, async (req, res) => {
  try {
    const battle = await db.get('SELECT * FROM battle_sessions WHERE id = $1', [parseInt(req.params.id)]);
...
router.post('/:id/move', authenticate, async (req, res) => {
  try {
    ...
    // 保存状态
    db(
      'UPDATE battle_sessions SET units_state = $1 WHERE id = $1'
...
router.post('/:id/end-turn', authenticate, async (req, res) => {
  try {
    ...
    db(
      'UPDATE battle_sessions SET units_state = $1, phase = $2 WHERE id = $3'
    )(JSON.stringify(nextState), nextState.phase, parseInt(req.params.id));
...
router.post('/:id/select-spawn', authenticate, async (req, res) => {
  try {
    ...
    db('UPDATE battle_sessions SET units_state = $1, phase = $2, spawn_order = $3 WHERE id = $1')
      (JSON.stringify(state), state.phase, JSON.stringify(state.spawn_order), parseInt(req.params.id));
...
router.post('/:id/end-deployment', authenticate, async (req, res) => {
  try {
    ...
    db('UPDATE battle_sessions SET units_state = $1, phase = $2 WHERE id = $3')
      (JSON.stringify(state), state.phase, parseInt(req.params.id));
...
router.post('/:id/end-tactical', authenticate, async (req, res) => {
  try {
    ...
    db('UPDATE battle_sessions SET units_state = $1, phase = $2, current_faction = $4, turn_number = $5 WHERE id = $1')
      (JSON.stringify(state), state.phase, state.current_faction, state.turn_number, parseInt(req.params.id));
