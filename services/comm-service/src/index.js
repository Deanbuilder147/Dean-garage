...
app.post('/api/comm/rooms', authenticate, async (req, res) => {
  ...
  try {
    ...
    if (!combatRes.ok) {
      ...
      console.error('Combat-Service 创建战斗失败:', errorData);
      return res.status(500).json({ error: '创建战斗会话失败', details: errorData });
    }
...
app.get('/api/comm/rooms/:roomId', authenticate, async (req, res) => {
  ...
  try {
    // 获取战场详情
    const mapRes = await fetch(`${MAP_SERVICE_URL}/api/map/battlefields/${roomState.battlefieldId}`);
    if (mapRes.ok) {
