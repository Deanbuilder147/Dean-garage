...
function _migrateExistingTablesInner(): void {
  ...
  if (!unitColNames.includes('size')) {
    try { db.run("ALTER TABLE units ADD COLUMN size TEXT DEFAULT 'm'"); console.log('[DB] 迁移：units 表添加 size 列'); } catch (e: any) { if (!e?.message?.includes('duplicate column')) throw e; }
  }
