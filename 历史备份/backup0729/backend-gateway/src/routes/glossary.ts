...
function normalizeSkillForSave(key: string, raw: any): any {
  ...
  if (skill.trigger != null && skill.trigger !== '') {
    ...
    console.warn(`[Glossary][A4] 词条 "${key}" 携带 trigger="${skill.trigger}"，该字段为死字段，不参与任何调度，请勿依赖。`);
  }
  return skill;
}
...
router.post('/config', authenticate, (req, res) => {
  try {
    ...
    if (isAdmin) {
      ...
      console.log(`[Glossary] 普通用户 ${req.auth?.username || 'guest'} 提交的 skills 改动已忽略（需 admin）`);
    }
