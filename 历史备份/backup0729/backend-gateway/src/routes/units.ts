...
router.post('/api/units', (req, res) => {
  try {
    ...
    console.log(`[Units] 创建: ${name} (is_public=${finalIsPublic}, review=${review_status}, size=${normSize(size)})`);
...
router.post('/api/units/upload-view', imageUpload.single('image'), (req, res) => {
  try {
    ...
    console.log(`[Units/View] 七视图上传: ${key}_${direction} -> ${filename}`);
...
router.post('/api/units/factions', imageUpload.single('image'), (req, res) => {
  try {
    ...
    persistChanges();
    console.log(`[Units/Faction] 阵营创建成功: ${code} (${name})`);
