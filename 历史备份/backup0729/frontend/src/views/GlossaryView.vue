                ...
                <div class="block-body">
                  <template v-for="def in compatFieldDefs" :key="def.key">
                    <label class="param-row" :for="`skill-${key}-${def.key}`">
                      ...
                      <input v-if="editMode && def.type==='number'" :id="`skill-${key}-${def.key}`" v-model.number="skill[def.key]" type="number" :min="def.min" :max="def.max" class="param-input" />
                      <select v-else-if="editMode && def.type==='select'" :id="`skill-${key}-${def.key}`" v-model="skill[def.key]" class="param-select">
                        <option v-for="o in def.options" :key="o.value" :value="o.value">{{ o.label }}</option>
                      </select>
                      <input v-else-if="editMode && def.type==='bool'" :id="`skill-${key}-${def.key}`" type="checkbox" v-model="skill[def.key]" />
                      <input v-else-if="editMode && def.type==='json'" :id="`skill-${key}-${def.key}`" :value="jsonStr(skill[def.key])" @input="e => setJson(skill, def.key, e.target.value)" class="param-input param-json" />
                      <input v-else-if="editMode" :id="`skill-${key}-${def.key}`" v-model="skill[def.key]" class="param-input" />
