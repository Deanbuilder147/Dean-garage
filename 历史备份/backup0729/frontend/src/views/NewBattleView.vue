...
import { applySizeMobility, sizeRenderScale, normSize, SIZE_LABELS } from '../utils/unitSize.js'
...
function unitSnapshot(u) {
  ...
  return {
    ...
    faction: u.faction,
    hp: u.hp ?? u.currentStats?.hp ?? 0,
    maxHp: u.maxHp ?? u.currentStats?.maxHp ?? 0,
...
function drawBattleScene(ctx, opts) {
  ...
  unitsWithScreenY.forEach(({ unit, flatX, flatY }) => {
    ...
    if (_hasSeven) {
      ...
      const maxH = HEX_RADIUS * 1.7 * sizeScale, maxW = HEX_RADIUS * 1.5 * sizeScale
      const aspect = iw / ih
      let dw, dh
...
async function executeAttack(target) {
  ...
  try {
    const duelRes = await combatAPI.duelCheck(route.params.id, {
      caster_unit_id: String(attacker.id),
      target_unit_id: String(target.id),
    })
...
async function confirmDuel() {
  ...
  try {
    const res = await combatAPI.resolveDuel(route.params.id, {
      caster_unit_id: d.attackerId,
      target_unit_id: d.defenderId,
    })
...
async function resolveSnatch(accept) {
  ...
  try {
    await combatAPI.resolveSnatch(route.params.id, {
      caster_unit_id: s.attackerId,
      target_unit_id: s.targetId,
      accept,
    })
  } catch (e) {
