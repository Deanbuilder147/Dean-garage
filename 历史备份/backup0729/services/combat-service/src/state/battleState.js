...
// 方案A：优先读 glossary terrains[tid].move_cost（单一真相源 S3），缺失回退 TERRAIN_COST_MAP。
// 以 CJS require 方式加载 configLoader（与网关掩体减伤同源），ESM 上下文无 require 时自动降级为兜底，不崩溃。
let _glossaryGetTerrainConfig = null;
try {
  if (typeof require !== 'undefined') {
    _glossaryGetTerrainConfig = require('../services/combatCore/configLoader.cjs').getGlossaryConfig;
  }
} catch {
  _glossaryGetTerrainConfig = null;
}

function getTerrainCost(terrainId) {
  if (_glossaryGetTerrainConfig) {
    try {
      const terrains = _glossaryGetTerrainConfig()?.terrains;
      const def = terrains && terrains[terrainId];
      if (def && typeof def.move_cost === 'number') return def.move_cost;
    } catch {
      // 读取异常则走兜底
    }
  }
